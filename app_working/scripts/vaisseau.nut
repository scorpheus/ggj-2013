...
choice_inertie_horizontal = 0
choice_inertie_vertical = 0
	//------------------
...
*/
		...
		if(find_a_friend)
		{				
			local value_acceleration = 10.0
			//change random stuff everyrandom stuff
			if(rand()%30 >15)
			{			
				if(rand()%30 > 15)
					choice_inertie_horizontal = false
				else
					choice_inertie_horizontal = true
					
				inertie_vec.x = 0.0
				// left
				if(choice_inertie_horizontal)
					inertie_vec += Vector(-value_acceleration,0.0,0.0) * g_dt_frame
				else
				// right
					inertie_vec += Vector(value_acceleration,0.0,0.0) * g_dt_frame		
			}
			...
			{			
				if(rand()%30 > 15)
					choice_inertie_vertical = false
				else
					choice_inertie_vertical = true
				
				inertie_vec.y = 0.0
				// up
				if(choice_inertie_vertical)
					inertie_vec += Vector(0.0,value_acceleration,0.0) * g_dt_frame
				else
				// down
					inertie_vec += Vector(0.0,-value_acceleration,0.0) * g_dt_frame			
			}
