...
{		
	...
	function	RefreshValueText(_text, _force=false)
	{
		...
		{ 
			local new_rect = TextComputeRect(Rect(0,0,9999,9999), _text.tostring()+" "+unit.tostring(), "simu_font", { size = font_size })
		...
		PictureTextRender(array_picture["text_part"], Rect(0, 10, SpriteGetSize(array_sprite["text_part"]).x, SpriteGetSize(array_sprite["text_part"]).y),  text.tostring()+" "+unit.tostring(), "simu_font" ,{ size = font_size, color = font_color })					
	...
	{				
		...
		local MiddleRect = TextComputeRect(Rect(0,0,9999,9999), _text.tostring()+" "+unit.tostring(), "simu_font", { size = font_size })
		if(authorize_background)
		...
		// blit text part
		{
			...
			PictureTextRender(pict, Rect(0, 10, MiddleRect.ex, MiddleRect.ey), _text.tostring()+" "+unit.tostring(), "simu_font" ,{ size = font_size, color = font_color })
